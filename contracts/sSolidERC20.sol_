...
import "./interfaces/IgSOLID.sol";
import "./interfaces/IsSOLID.sol";
import "./interfaces/IStaking.sol";

contract sSolid is IsSOLID, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgSOLID public gSOLID; // additional staked supply (governance token)

    Rebase[] public rebases; // past rebase data

    uint256 private constant MAX_UINT256 = type(uint256).max;

    //TODO: Stenver question: what would be FRAGMENTS? Would it be the tokens of the initial offers?
    ...
    constructor() ERC20("Staked SOLID", "sSOLID", 9) ERC20Permit("Staked SOLID") {
    ...
    function setgSOLID(address _gSOLID) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gSOLID) == address(0), "gSOLID:  gSOLID already set");
        require(_gSOLID != address(0), "gSOLID:  gSOLID is not a valid contract");
    ...
    // toG converts an sSOLID balance to gSOLID terms. gSOLID is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gSOLID.balanceTo(amount);
    }

    // fromG converts a gSOLID balance to sSOLID terms. sSOLID is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gSOLID.balanceFrom(amount);
    }

    // Staking contract holds excess sOHM
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gSOLID.balanceFrom(IERC20(address(gSOLID)).totalSupply())).add(
