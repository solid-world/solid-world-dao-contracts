...
contract DecollateralizationManagerTest is BaseSolidWorldManagerTest {
    ...
    function testGetBatchesDecollateralizationInfoWithCertificationDate() public {
        _addCategoryAndProjectWithApprovedSpending(CATEGORY_ID, PROJECT_ID, TIME_APPRECIATION);
        _addCategoryAndProjectWithApprovedSpending(CATEGORY_ID + 1, PROJECT_ID + 1, TIME_APPRECIATION);
        _addBatchWithCertificationDateToProject(BATCH_ID, PROJECT_ID, PRESET_CURRENT_DATE + 2 * ONE_YEAR);
        _addBatchWithCertificationDateToProject(BATCH_ID + 1, PROJECT_ID, PRESET_CURRENT_DATE + ONE_YEAR);
        _addBatchWithCertificationDateToProject(BATCH_ID + 2, PROJECT_ID, PRESET_CURRENT_DATE + ONE_YEAR);
        ...
        manager.collateralizeBatch(BATCH_ID, 5000, 0);
        manager.collateralizeBatch(BATCH_ID + 1, 5100, 0); // 16-10-2023 14:25:43 UTC
        manager.collateralizeBatch(BATCH_ID + 2, 5200, 0); // 16-10-2023 14:25:43 UTC
        manager.collateralizeBatch(BATCH_ID + 3, 5300, 0);
        manager.collateralizeBatch(BATCH_ID + 4, 5400, 0);
        ...
        DomainDataTypes.TokenDecollateralizationInfo[] memory info = manager
            .getBatchesDecollateralizationInfoWithCertificationDate(PROJECT_ID, 1697466343);
